...
function CakeAdapterAction (&$amfbody) {
	...
	if (strpos($target, "http://") === false && strpos($target, "https://") === false) { // check for a http link which means web service
		...
		if ($lpos === false) {
			...
			if($messageType == 'flex.messaging.messages.RemotingMessage')
			{
				...
				$uriclasspath = str_replace('.','/',Inflector::underscore($classAndPackage)) . '.php';
			...
			if ($lpos === false) {
				...
				else if($classname == "PageAbleResult" && $methodname == 'release')
				{
					...
					$uriclasspath = Inflector::underscore($trunced) . ".php";
					$classpath = $baseClassPath . Inflector::underscore($trunced) . ".php";
				} 
			} else {
				$classname = substr($trunced, $lpos + 1);
				$classpath = $baseClassPath . Inflector::underscore(str_replace(".", "/", $trunced)) . ".php"; // removed to strip the basecp out of the equation here
				$uriclasspath = Inflector::underscore(str_replace(".", "/", $trunced)) . ".php"; // removed to strip the basecp out of the equation here
			} 
		}
	...
	$amfbody->className = Inflector::camelize($classname);
 ...
 */
function CakeExecutionAction (&$amfbody) 
{	
